...
class MultiModalBartDecoder_span(nn.Module
    ...
    def forward(self, tokens, state, only_sc=False):
        ...
        if self.need_tag:  # Babu
            tag_scores = F.linear(
                hidden_state,
                self.dropout_layer(
                    self.decoder.embed_tokens.weight[self.label_start_id:self.label_start_id + 3]))  # bsz x max_len x num_class
            # print(self.src_start_index)
            # print(tag_scores)
            # print(tag_scores.shape)
            logits[:, :, self.src_start_index-3:self.src_start_index] = tag_scores
            # logits[i,j,ssi-3:ssi] = 样本i的decoder输出的第j个token embedding与<<POS>><<NEU>><<NEG>>的token embedding相似度
