...
# 使用余弦衰减计算学习率衰减
def cosine_decay_learning_rate(step, total_steps, alpha=0.0):
    cosine_decay = 0.5 * (1 + np.cos(np.pi * step / total_steps))
    decayed_lr = (1 - alpha) * cosine_decay + alpha
    return decayed_lr

def liner_warmup_cosine_decay(cur_step, t_step, warmup):
    progress = cur_step / t_step
    if progress < warmup:
        return progress / warmup
    return cosine_decay_learning_rate(cur_step - t_step * warmup, t_step - t_step * warmup, 0.0)
